...
public final class TeleportEffectService {
    ...
    public void startCharging(Player player, WaystoneData origin) {
        ...
        player.playSound(base.clone().add(0.5, 1.0, 0.5), Sound.BLOCK_BEACON_ACTIVATE, 0.8f, 1.15f);
        ...
        BukkitTask task = plugin.getServer().getScheduler().runTaskTimer(plugin, new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < 12; i++) {
                    ...
                    world.spawnParticle(Particle.ENCHANT, center.clone().add(x, 0.2, z), 1, 0.02, 0.04, 0.02, 0.0);
                    if ((i & 1) == 0) {
                        world.spawnParticle(Particle.PORTAL, center.clone().add(x * 0.8, 0.45, z * 0.8), 1, 0.01, 0.08, 0.01, 0.0);
                    }
                }

                for (double y = 0.35; y <= height; y += 0.55) {
                    world.spawnParticle(Particle.END_ROD, center.clone().add(0.0, y, 0.0), 1, 0.025, 0.06, 0.025, 0.0);
                }

                if (tick % 5 == 0) {
                    world.spawnParticle(Particle.ELECTRIC_SPARK, center.clone().add(0, 1.0, 0), 3, 0.35, 0.65, 0.35, 0.02);
                }
                tick++;
            }
        }, 0L, 4L);
    ...
    public void pulse(Player player, WaystoneData origin) {
        ...
        Location base = origin.location();
        if (base == null || base.getWorld() == null) return;
        base.getWorld().playSound(base.clone().add(0.5, 1.0, 0.5), Sound.BLOCK_AMETHYST_BLOCK_RESONATE, 0.5f, 1.0f + (float) (Math.random() * 0.18));
    }
    ...
    public void cancel(Player player, WaystoneData origin) {
        ...
        base.getWorld().spawnParticle(Particle.ASH, center, 18, 0.55, 0.45, 0.55, 0.02);
        base.getWorld().playSound(center, Sound.BLOCK_BEACON_DEACTIVATE, 0.55f, 0.9f);
    }
    ...
    private void burst(Location base, boolean departure) {
        ...
        world.spawnParticle(Particle.END_ROD, center, 42, 0.65, 1.1, 0.65, 0.055);
        world.spawnParticle(Particle.PORTAL, center, 64, 0.8, 1.0, 0.8, 0.18);
        world.spawnParticle(Particle.ELECTRIC_SPARK, center, 20, 0.65, 0.9, 0.65, 0.07);
        world.playSound(center, departure ? Sound.BLOCK_BEACON_POWER_SELECT : Sound.BLOCK_PORTAL_TRAVEL,
                departure ? 0.9f : 0.65f, departure ? 1.35f : 1.15f);
    }
